...
static esp_err_t http_server_get_dht_sensor_readings_json_handler(httpd_req_t *req)
{
	...
	int len_dhtSensorJSON = sprintf(dhtSensorJSON, "{\"temp\":\"%d\",\"raw_voltage\":\"%d\"}", temp, 10);

	httpd_resp_set_type(req, "application/json");
	httpd_resp_send(req, dhtSensorJSON, len_dhtSensorJSON);

	return ESP_OK;
}
static esp_err_t http_server_get_pot_sensor_readings_json_handler(httpd_req_t *req)
{
	ESP_LOGI(TAG, "/potSensor.json requested");

	char potSensorJSON[100];
	float voltage;
	//int pot;
	xQueueReceive(VOL_Q,&voltage,100/ portTICK_PERIOD_MS);
	
	int len_potSensorJSON = sprintf(potSensorJSON, "{\"vol\":\"%f\"}", voltage);

	httpd_resp_set_type(req, "application/json");
	httpd_resp_send(req, potSensorJSON, len_potSensorJSON);
...
static esp_err_t http_server_toogle_uart_handler(httpd_req_t *req)
{
	...
	// }
	int status_off=2;
	xQueueSend(GET_TEMP_STATUS, &status_off,0);
...
static esp_err_t http_server_restart_uart_handler(httpd_req_t *req)
{
	// if (tempTaskHandle==NULL){
	// xTaskCreatePinnedToCore( &GET_TEMP_FUNCTION , "uart ntc" , 2048 , NULL, 5, &tempTaskHandle , 1 );
	// }}
	int status_on=1;
	xQueueSend(GET_TEMP_STATUS, &status_on,0);
...
static httpd_handle_t http_server_configure(void)
{
	...
	if (httpd_start(&http_server_handle, &config) == ESP_OK)
	{
		...
		httpd_uri_t pot_sensor_json = {
				.uri = "/potSensor.json",
				.method = HTTP_GET,
				.handler = http_server_get_pot_sensor_readings_json_handler,
				.user_ctx = NULL
		};
		httpd_register_uri_handler(http_server_handle, &pot_sensor_json);
		...
		httpd_uri_t restart_u = {
			.uri = "/uart_on.json",
			.method = HTTP_POST,
			.handler = http_server_restart_uart_handler,
			.user_ctx = NULL
		};
		httpd_register_uri_handler(http_server_handle, &restart_u);
